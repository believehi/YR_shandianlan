	...
	<view class="content">
		...
		<view class="uni-tx-content">
			<view class="uni-tx-item"  v-for="(item,index) in userfoArray" :key="index">
				...
				<text class="uni-tx-zt">{{item.State == 1 ? '待审核' :(item.State ==2 ?  '已完成' :'已失败')}}</text>
			</view>	
			<text  :class="{tisi:tishi == '暂无充值记录'}" >{{tishi}}</text>
		</view>
	...
	export default {
		...
		methods: {
			srlist() {
				...
				uni.request({
					url: helper.websiteUrl + 'yiru/sdl/translation/getlist',
					method: 'GET',
...
<style>
	...
	.uni-tx-item{display: flex;justify-content: center;border-top: 1upx solid #e6e6e6;align-items: flex-end;font-size: 30upx;padding: 20upx 0;display:flex;justify-content: space-between;text-align: center;align-items: center;}
	.uni-tx-item:first-child{border: none !important;}
	.uni-tx-sum{width: 20%;}
