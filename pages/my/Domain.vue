<template>
	<view class="content">
		<view class="uni-item">
			<view class="uni-domain-list" v-for="(item,index) in PlanArray" :key="index" :data-ptid="item.ID" @click="opendomain">
				<text>{{item.PlanName}}</text>
				<text class="uni-pt">{{item.UserName}}</text>
		...
		methods: {
			...
			Domaininfo() {
				uni.showLoading({
					...
					uni.request({
						url: helper.websiteUrl + 'plan/userplanlist',
						method: 'GET',
